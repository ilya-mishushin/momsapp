...
const AlbumView: React.FC = () => {
    ...
    const handleDeletePhoto = async (photoId: number) => {
        try {
            await axios.delete(`${process.env.REACT_APP_API_URL}/photo/${photoId}`);
    ...
    const handleRenamePhoto = async (photoId: number, newName: string) => {
        try {
            await axios.put(`${process.env.REACT_APP_API_URL}/photo/${photoId}`, { filename: newName });
    ...
    const handleMovePhoto = async (photoId: number, newAlbumName: string) => {
        ...
        try {
            await axios.post(`${process.env.REACT_APP_API_URL}/albums/${newAlbum.id}/photos`, { photoId });
            setPhotos(photos.filter(photo => photo.id !== photoId));
        } catch (err) {
