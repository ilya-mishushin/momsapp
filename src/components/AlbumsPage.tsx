...
const AlbumsPage: React.FC = () => {
    ...
    const apiUrl = 'http://80.234.78.166:3000';
        ...
        const fetchAlbums = async () => {
            try {
                ...
                const photosPromises = response.data.map(async (album: Album) => {
                    const photosResponse = await axios.get(`${apiUrl}/albums/${album.id}/photos`);
    ...
    const confirmDeleteAlbum = async () => {
        if (albumToDelete) {
            try {
                await axios.delete(`${apiUrl}/albums/${albumToDelete}`);
    ...
    const handleEditAlbum = async (id: number) => {
        ...
        if (newName) {
            try {
                const response = await axios.put(`${apiUrl}/albums/${id}`, {
                    name: newName
                });
